...
export function fetchBatchBalances(): Promise<BatchBalanceApiResponse> {
...
export function submitTimeOffRequest(
  ...
  return apiFetch(`${BASE}/requests`, {
    ...
    headers: {
      ...
      ...(scenario ? { "x-pto-scenario": scenario } : {}),
    },
    body: JSON.stringify(body),
  });
}

export function fetchTimeOffRequests(employeeId?: string): Promise<{ requests: TimeOffRequest[] }> {
  const qs = employeeId ? `?employeeId=${encodeURIComponent(employeeId)}` : "";
  return apiFetch(`${BASE}/requests${qs}`);
}

export function fetchTimeOffRequest(id: string): Promise<TimeOffRequest> {
  return apiFetch(`${BASE}/requests/${encodeURIComponent(id)}`);
}

export function approveTimeOffRequest(id: string): Promise<ApprovalResponse> {
  return apiFetch(`${BASE}/requests/${encodeURIComponent(id)}`, {
    method: "PATCH",
...
export function denyTimeOffRequest(id: string): Promise<ApprovalResponse> {
  return apiFetch(`${BASE}/requests/${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export function triggerAnniversaryBonus(
  employeeId: string,
  locationId: string
): Promise<{ message: string; balance?: BalanceApiResponse }> {
